...
def upload_binary(io, local_path: Path, remote_path: str) -> None:
    ...
    send_cmd(io, f"rm -f {remote_path}.b64")

    for i in range(0, len(blob), chunk_size):
        print(f'uploading chunk {i // chunk_size} ({i})')
        chunk = blob[i:i + chunk_size]
        io.sendline(f"cat >> {remote_path}.b64 <<'EOF'".encode())
        io.send(chunk.encode() + b"\n")
        io.sendline(b"EOF")
        wait_prompt(io)

    send_cmd(
        io,
        f"base64 -d {remote_path}.b64 > {remote_path} && chmod +x {remote_path} && rm -f {remote_path}.b64",
    )
